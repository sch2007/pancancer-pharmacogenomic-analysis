...
saveLabel = '_n{}_k{}_KM{}{}'.format(niter, nfold, KM_max, extraLabel).replace('.','')
...
for drug in drugs:
    ...
    plt.title(drug, fontsize=fontSize+2)
    ...
    for iiter in range(1, niter+1):
        ...
        for iKM, KM in enumerate(KMs):
            ...
            for ifold in range(1, nfold+1):
                ...
                # 1) load Sensitivity and Specificity of training and validation
                sel = ((d1['drug'] == drug) &
        ...
        if 0:
            plt.plot(xx, yy_test[iiter-1, :], ':', color='seagreen')
    ...
    # Plotting
    plt.plot(xx, yy_trainval_avg, '--', color='steelblue')
    plt.plot(xx, yy_test_avg, '-', color='seagreen')
    ...
    plt.text(0.96, 0.06, r'Training: AUC = {:.2g} $\pm$ {:.2g}'.format(
            np.nanmean(AUC_trainval), np.nanstd(AUC_trainval)), fontsize=fontSize-2,
             horizontalalignment='right',  verticalalignment='bottom',
             color='steelblue', transform=plt.gca().transAxes)
    plt.text(0.96, 0.02, r'Testing: AUC = {:.2g} $\pm$ {:.2g}'.format(
            np.nanmean(AUC_test), np.nanstd(AUC_test)), fontsize=fontSize-2,
             horizontalalignment='right', verticalalignment='bottom',
             color='seagreen', transform=plt.gca().transAxes)
    ...
    if save_figure:
        plt.savefig('Cross_' + drug + saveLabel + '.pdf', bbox_inches='tight')
